...
module.exports = (() => {
  ...
  // const makeForAlphabet = alphabet => alphabet.reduce(
  ...
  const toHTML = DKAs => Object.entries(DKAs).map(([letter, dka]) => fsm.printHtmlTable(dka, letter)).join('</br>');

  const writeHTML = (DKAs) => {
    const html = `<!DOCTYPE html><html><head></head><body>${toHTML(DKAs)}</body></html>`;
    const fileName = `./out/dkas-${new Date().toLocaleTimeString()}.html`;

    appendFile(fileName, html, (err) => {
      if (err) {
  ...
  return {
    dkasForAlpabet, randomWord, isWordAccepted, writeHTML,
  };
})();
