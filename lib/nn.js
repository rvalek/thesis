...
module.exports = (() => {
  ...
  // generate random fsm
  const createRandomFsm = (numStates, numAlphabet, maxNumToStates = 1) => {
    let newFsm = {};
  ...
  const isStringInLanguage = (fsm, inputSymbolStream) => {
    ...
    return states.some(state => fsm.acceptingStates.includes(state));
  };
  ...
  return {
    createRandomFsm, makeEmpty, randomStringInLanguage, isStringInLanguage, printHtmlTable,
  };
})();
